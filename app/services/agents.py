...
from app.tools.predict_products import predict_top_products
import json
import re
...
_SYSTEM_PROMPT = (Path(__file__).parent.parent /
                  "prompts"/"sales_agent.txt").read_text()
agent = create_agent(
    model=ChatOpenAI(api_key=settings.openai_api_key, model="gpt-4.1-mini"),
    tools=[predict_sales, predict_top_products],
...
async def stream_generator(user_input: str) -> AsyncIterable[str]:
    try:
        ...
        response = re.sub(r'(?i)(top|next|last)(\d)',
                          r'\1 \2', "".join(full_response))
